...
/**
 * This program using recursive algorithm that reads two lines of a file, 
 * one defines the lights with letters, eg. A B C D,
 * another line defines a set of edges. eg. AB
 * generate the list of button that either turn off (or turn on) all the light, 
 * which is the optimal solution.
 * 
 * @author: Orlando Yuan, Usman Shah
 */
public class LightConrol {
	static int num = 10;
	...
	/** 
	 * main method set a scanner read input from file 
	 * using the return value of getInputList()
	 * calls optimal() to do further execution
	 * */
	...
	/** 
	 * method that split the input and prepare for further calculation
	 * by calling solve()
	 */
	...
	/**
	 * method that controls the status of lights
	 * the index = 1 is on, 0 is off
	 */
	public static void changeStaticOfLight(ArrayList<Character> index) {
		for (char lightSwitch : index) {
			// according to the current status of the lights
			if (lightMap.get(String.valueOf(lightSwitch)) == 1) {
				lightMap.put(String.valueOf(lightSwitch), 0);
			} else {
				lightMap.put(String.valueOf(lightSwitch), 1);
	...
	/** 
	 * method that does the final calculation 
	 * it will be called in optimal() 
	 * */
	...
	/**
	 * method that performs the recursive algorithm
	 * the return value will be used in solve()
	 */
	public static void backTrace(int cur) {
		...
		//cur starts at 0, in a case A B C D, AC AD BD CA when cur reaches 4 excute this loop
		...
		for (int i = 0; i < tempControl.get(cur).size(); i++) {
			if (solveLightMap.get(String.valueOf(tempControl.get(cur).get(i))) == 1) {
				solveLightMap.put(String.valueOf(tempControl.get(cur).get(i)),
						0);
			} else {
				solveLightMap.put(String.valueOf(tempControl.get(cur).get(i)),
						1);
